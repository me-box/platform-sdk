...
const insert = (obj, replacement) => {

	if (replacement.trim() === "") {
		return obj;
	}
	return obj.map(item => ({ ...item, required: [replacement] }));
}

export function extract_ptype_from_inputs(inputs, id = "", replace = "") {
	return inputs.reduce((acc, input) => {
		if (input.schema && input.schema.output && input.schema.output.ptype) {
			return {
				...acc,
				...Object.keys(input.schema.output.ptype).reduce((acc, key) => {
					const replaced = insert(input.schema.output.ptype[key], replace);
					return {
						...acc,
						[id]: [...(acc[id] || []), ...replaced]
					}
...
//from david walsh, taken from underscore.js
