...
export function createTestContainer(image, name){
	...
	return new Promise((resolve, reject)=>{
		docker.createContainer({Image: image, PublishAllPorts:true, Binds: ["/tmp/app.webserver:/tmp/app.webserver"], Links: ["mock-datasource:mock-datasource"], Env: ["TESTING=true", "MOCK_DATA_SOURCE=http://mock-datasource:8080"],  Labels: {'user':`${name}`}, "ExposedPorts": {"1880/tcp": {}}, Cmd: ["npm", "start", "--", "--userDir", "/data"], name: `${name}-red`}, function (err, container) {
			if (err){
				console.log(err);
				reject(err);
			}else{
